...
const Page = () => {
  ...
  const total = products.reduce(
    (acc, product) => acc + Number(product.price) * Number(product.serves || 1),
    0
  );
  ...
  return (
    <div className="w-full  min-h-[80vh] flex flex-col justify-start items-center mt-4">
      ...
      {isPaymentSuccessModalOpen && (
        <PaymentSuccessModel isPaymentloading={isPaymentLoading} />
      )}
      ...
      <div className="w-full   p-5 rounded-md flex flex-row gap-1">
            ...
            <>
              <h3 className="text-[12px] font-bold mb-2 flex flex-row justify-between">
                ...
                <div className="flex flex-row items-center justify-center gap-2">
                  ...
                  />
                  <RiDeleteBinLine
                    onClick={() => setIsRemoveAddressModalOpen(true)}
              ...
              <ul className="text-[10px] font-semibold flex flex-col gap-1">
                <li>
                  {userData?.shippingAddress.shippingName || "Name Not Found"}
                </li>
                <li>
                ...
                <li>
                  {userData?.shippingAddress.shippingCity || "City Not Found"},{" "}
                  {userData?.shippingAddress.shippingCode || "Code Not Found"}
                </li>
                <li>
                  {userData?.shippingAddress.shippingCountry ||
                    "Country Not Found"}
                </li>
                <li>
                  {userData?.shippingAddress.shippingPhone || "Phone Not Found"}{" "}
                </li>
                <li>
                  {userData?.shippingAddress.shippingEmail || "Email Not Found"}
                </li>
              </ul>
            </>
          ) : (
        ...
        <div className="w-[calc(100%-30vh)] min-h-[20vh] p-5 flex flex-row items-center justify-center gap-2">
          <div className="w-[15vh] h-[15vh]  flex flex-col items-center justify-center">
            ...
            <span className="font-semibold text-md text-green-400 whitespace-nowrap ">
              User Verified
            </span>
          </div>
          ...
          <div
            className={`w-[15vh] h-[5px]  ${
              userData?.shippingAddress ? "bg-green-400" : "bg-red-300"
            } rounded-2xl`}
          ></div>
          ...
          <div className="w-[15vh] h-[15vh] flex flex-col items-center justify-center">
            <FaAddressCard
              className={`p-5  ${
                userData?.shippingAddress ? "bg-green-400" : "bg-red-300"
              } w-20 h-20 rounded-full text-white shadow-md`}
            />
            <span
              className={`font-semibold text-md ${
                userData?.shippingAddress ? "text-green-400" : "text-red-300"
              } whitespace-nowrap`}
            >
              {" "}
              {userData?.shippingAddress
          ...
          <div className="w-[400px] h-full max-w-md mx-auto mt-10 p-6  ">
            <div className="w-full h-[30vh] flex flex-col justyfy-start items-center gap-2 overflow-auto">
              {products.map((product) => (
                <div
                  key={product.id}
                  className="w-full h-[10vh] flex flex-row justify-between items-center"
                >
                  ...
                  <div className="w-[15vh] h-[10vh] flex flex-col justify-end items-start gap-2 font-semibold text-sm pl-6 pb-4 ">
                    ...
                    <span>
                      {product.serves || "1"} * {product.price} LKR
                    </span>
                  </div>
                  <div className="w-[10vh] h-[10vh] flex flex-col justify-end items-center font-semibold text-sm pb-4 ">
                    {Number(product.serves || 1) * Number(product.price)} LKR
                  </div>
                </div>
              ))}
            </div>
